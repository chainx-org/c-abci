/*********************************************************************************************/
/*                         tcp.h Created by xuli on 17/02/10                                 */
/*                    注:所有的socket fd都已经设置成了非阻塞模式                             */
/*********************************************************************************************/
#ifndef __TCP_H__
#define __TCP_H__
...
/************************************************************************************************
 * 绑定和监听一个IP地址端口
 * @socket:输出参数，由本函数填充结构体值
 * @ipaddr:待绑定监听的本端IP地址(NULL代表绑定的地址为INADDR_ANY)
 * @port:待绑定监听的本端IP端口
 * @return: 成功返回0，失败返回-1(错误码返回给socket中的error)
 ************************************************************************************************/
 ...
 * @ipaddr:要连接的对端IP地址
 * @port:要连接的对端IP端口
 * @timeout:连接对端超时设置 <0:使用默认超时时间 0:不设置超时 >0:具体的超时时长(单位ms)
 * @return: 成功返回0，失败返回-1(错误码返回给socket中的error)
 ************************************************************************************************/
